...
jQuery( document ).ready( function() {
    ...
    if (popupShowed === 'true') {
        // $(".wpcf7").find('input[type="submit"]').removeAttr('disabled');
        //$(".rm-form .wpcf7").find('input[type="submit"]').attr('disabled', 'disabled');
    } else{
        // $(".rm-form .wpcf7").find('input[type="submit"]').attr('disabled', 'disabled');
    }
    ...
    $('.rm-form select').styler();
    $('.intro2-right_form select').styler();
    $('.bottom7-contact select').styler();
	$('.contact3 select').styler();


    $('.scroll_bottom a').on('click', function (e) {
		e.preventDefault();
		$("html, body").animate({
    ...
    $('.scroll_footer a').on('click', function (e) {
        e.preventDefault();
        $("html, body").animate({
    ...
    $('#page.hfeed').find('#content article .intro-section .learn-more > a').on('click', function(e){
    	e.preventDefault();

        header_height = $('.rm-header-bottom').innerHeight();
        form_offset = $('#page.hfeed').find('#content article .entry-content > div:nth-child(5) .wpb_column').offset();
        form_offset = form_offset.top;
        if($(window).width() >= 990)
		{
            form_offset = form_offset - header_height - 20;
		}else
		{
            form_offset = form_offset - header_height - 58;
		}
...
jQuery( window ).load( function() {
    ...
    $(".kitchen_icons .image_icon").each(function(){
        if($(this).find('.vc_column-inner').outerHeight() > icon_height) {
            icon_height = $(this).outerHeight();
        }
    });
    ...
    $(".rm7_slider").owlCarousel({
        items: 1,
        autoplay: false,
		dots: false,
		nav: true,
        navText: ['<img src="https://1800remodel.com/wp-content/themes/tm-renovation/images/Arrow_white.svg">', '<img src="https://1800remodel.com/wp-content/themes/tm-renovation/images/Arrow_white.svg">'],
		loop: true
    });
...
jQuery( window ).resize( function() {
    ...
    $(".kitchen_icons .image_icon").each(function(){
        if($(this).find('.vc_column-inner').outerHeight() > icon_height) {
            icon_height = $(this).find('.vc_column-inner').outerHeight();
        }
    });
    ...
    $('.btn.thememove-btn').on('click', function(){
        $('html, body').animate({scrollTop:$('.rm-form').position().top}, 2000);
        return false;
    });
...
jQuery(document).ready( function() {
    ...
    jQuery('.partners_page a[href^="#"]').on('click', function(e){
        jQuery('html,body').stop().animate({ scrollTop: jQuery('#cntctfrm').offset().top }, 1000);
        e.preventDefault();
    });


	// jQuery("#header_partners_wrap").sticky({topSpacing:0});
	// jQuery(window).on('scroll', function(){
	// 	console.log(jQuery(this).scrollTop());
	// 	if(jQuery(this).scrollTop()>300)
	// 	{
     //        jQuery('#partnersStickyHeader').fadeIn();
	// 	}else{
     //        jQuery('#partnersStickyHeader').fadeOut();
	// 	}
    //
	// });
});
